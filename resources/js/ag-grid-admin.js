...
(function initializeHotelioAgGrid() {
    ...
    function getNestedValue(source, path) {
        ...
        return String(path).split('.').reduce(function reducePath(currentValue, key) {
            return currentValue === null || currentValue === undefined ? undefined : currentValue[key];
        }, source);
    }

    function getHeadersFromTable(tableElement) {
        return Array.from(tableElement.querySelectorAll('thead th')).map(function mapHeader(header, index) {
    ...
    function getFilename(options, tableElement) {
        ...
        return makeSafeName(
            ...
            tableElement.id || tableElement.getAttribute('data-grid-name') || document.title
        );
    }

    function normalizeColumnDefinitions(options, tableElement) {
        const headers = getHeadersFromTable(tableElement);
        const configuredColumns = Array.isArray(options.columns) && options.columns.length
            ? options.columns
        ...
        return configuredColumns.map(function mapColumn(column, index) {
            const headerName = column.title || column.name || headers[index] || column.data || `Column ${index + 1}`;
            const fieldName = column.data || makeSafeName(headerName, `column-${index + 1}`);
            const isActionColumn = fieldName === 'action' || /action/i.test(headerName);
            ...
            return {
                ...
                cellClass: isActionColumn ? 'hotelio-ag-cell-actions' : undefined,
                cellRenderer: function renderCell(params) {
                    ...
                    if (looksLikeHtml(renderedValue)) {
                        ...
                        wrapper.className = isActionColumn ? 'hotelio-ag-actions' : '';
    ...
    function parseStaticRows(tableElement, columnDefinitions) {
        return Array.from(tableElement.querySelectorAll('tbody tr')).map(function mapRow(row) {
            const cells = Array.from(row.children);

            return columnDefinitions.reduce(function reduceCells(result, columnDefinition, index) {
                result[columnDefinition.field] = cells[index] ? cells[index].innerHTML.trim() : '';
    ...
    function getExportMatrix(gridApi, columnDefinitions) {
        const visibleColumns = columnDefinitions.filter(function filterVisible(columnDefinition) {
    ...
    function copyGrid(gridApi, columnDefinitions) {
        ...
        const text = [matrix.header].concat(matrix.body).map(function mapLine(line) {
            return line.join('\t');
        }).join('\n');
    ...
    function exportJson(gridApi, filename) {
        downloadBlob(
            new Blob([JSON.stringify(getVisibleRows(gridApi), null, 2)], { type: 'application/json' }),
            `${filename}.json`
        );
    }
    ...
    function createToolbar(gridApi, columnDefinitions, filename, reloadRows) {
        ...
        toolbar.querySelector('.hotelio-grid-search').addEventListener('input', function handleSearch(event) {
    ...
    function initializeGrid(tableElement, options, loadRows) {
        if (!tableElement || gridInstances.has(tableElement) || tableElement.closest('.modal')) {
            return gridInstances.get(tableElement);
        }

        const columnDefinitions = normalizeColumnDefinitions(options, tableElement);
        ...
        const reloadRows = function reloadRows() {
            return loadRows().then(function setRows(rows) {
    ...
    function createAjaxGrid(tableElement, options) {
        const ajaxOptions = typeof options.ajax === 'string'
        ...
        return initializeGrid(tableElement, options, function loadAjaxRows() {
            ...
            return new Promise(function resolveAjax(resolve, reject) {
                jquery.ajax({
                    ...
                    dataType: 'json',
                    data: Object.assign({}, ajaxOptions.data || {}, { grid: 'ag' }),
    ...
    function createStaticGrid(tableElement) {
        return initializeGrid(tableElement, {}, function loadStaticRows() {
            const columnDefinitions = normalizeColumnDefinitions({}, tableElement);

            return Promise.resolve(parseStaticRows(tableElement, columnDefinitions));
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function initializeStaticTables() {
        ...
        document.querySelectorAll(selectors.join(',')).forEach(function initializeStaticTable(tableElement) {
            if (
                !gridInstances.has(tableElement)
                && !tableElement.classList.contains('hotelio-grid-source')
    ...
    window.HotelioAgGrid = {
        refresh: function refresh(selector) {
            document.querySelectorAll(selector).forEach(function refreshTable(tableElement) {
